{dp}`irm_QM9QVMFbGpAF` {ts}`unmapped_with_rationale`
ISO 26262-6:2018 to Rust 1.93.1

{dp}`irm_UTF2ZsvZsk1d` {ts}`unmapped_with_rationale`
Complete Language and Standard Library Mapping

{dp}`irm_2wWFv6Epqk8O` {ts}`unmapped_with_rationale`
Reference for Safety-Critical Rust Coding Standards (QM to ASIL D)

{dp}`irm_0eRuopfOJASq` {ts}`unmapped_with_rationale`
Revision 2 — February 2026

{dp}`irm_uzh3Jx8axMMn` {ts}`unmapped_with_rationale`
Working reference for Safety-Critical Rust Consortium — Coding Guidelines Subcommittee

{dp}`irm_uuKgVYTQqHib` {ts}`unmapped_with_rationale`
Baseline: Rust stable 1.93.1 (2026-02-12), Edition 2024 (no nightly features)

# Table of Contents

{dp}`irm_IaS36rlingCN` {ts}`unmapped_with_rationale`
...
{dp}`irm_LCKDSuxTJuZk` {ts}`unmapped_with_rationale`
...
{dp}`irm_smKEjfQxUIDP` {ts}`unmapped_with_rationale`
...
{dp}`irm_V0aEY5D6LDGk` {ts}`unmapped_with_rationale`
...
{dp}`irm_gKLV60XC5i61` {ts}`unmapped_with_rationale`
...
{dp}`irm_KOJfIkMZDBqx` {ts}`unmapped_with_rationale`
...
{dp}`irm_AZXBxGsntM44` {ts}`unmapped_with_rationale`
...
{dp}`irm_Q0kgwYYR171s` {ts}`unmapped_with_rationale`
...
{dp}`irm_vgLkj9nsdzu2` {ts}`unmapped_with_rationale`
...
{dp}`irm_nVs90Ld3aPxU` {ts}`unmapped_with_rationale`
...
{dp}`irm_Ljp7mGth7Amy` {ts}`unmapped_with_rationale`
...
{dp}`irm_kODUEkMym4Os` {ts}`unmapped_with_rationale`
...
{dp}`irm_3za0ocrwtBGp` {ts}`unmapped_with_rationale`
...
{dp}`irm_ZeSGSHiyEroq` {ts}`unmapped_with_rationale`
...
{dp}`irm_eezCzmrly9Za` {ts}`unmapped_with_rationale`
...
{dp}`irm_SjTim8sW3zJR` {ts}`unmapped_with_rationale`
...
{dp}`irm_qMzaCkUGAJov` {ts}`unmapped_with_rationale`
...
{dp}`irm_St3R4bVtd1Co` {ts}`unmapped_with_rationale`
...
{dp}`irm_E9zH6MfjsDoZ` {ts}`unmapped_with_rationale`
...
{dp}`irm_8jygFLYHoZCv` {ts}`unmapped_with_rationale`
...
{dp}`irm_mhkgg0UAlpOc` {ts}`unmapped_with_rationale`
...
{dp}`irm_Fh2P6yxA9I4J` {ts}`unmapped_with_rationale`
...
{dp}`irm_9OOKDXtR6Svj` {ts}`unmapped_with_rationale`
...
{dp}`irm_G4ud2UzkRGFy` {ts}`unmapped_with_rationale`
...
{dp}`irm_LcwMqUtRwhIj` {ts}`unmapped_with_rationale`
...
{dp}`irm_lhyM4xSKQd1i` {ts}`unmapped_with_rationale`
...
{dp}`irm_Rwk3TjMDnmiA` {ts}`unmapped_with_rationale`
...
{dp}`irm_SHX3ExwzXLVD` {ts}`unmapped_with_rationale`
...
{dp}`irm_17W1UCto4gWA` {ts}`unmapped_with_rationale`
...
{dp}`irm_H5jGWNri8Om0` {ts}`unmapped_with_rationale`
...
{dp}`irm_cqpRAzKJFkiH` {ts}`unmapped_with_rationale`
...
{dp}`irm_XoOmi9gflWFB` {ts}`unmapped_with_rationale`
...
{dp}`irm_V4Aqq7q4lutJ` {ts}`unmapped_with_rationale`
...
{dp}`irm_rnKNXqR7kwFi` {ts}`unmapped_with_rationale`
...
{dp}`irm_WfMViced3Oiy` {ts}`unmapped_with_rationale`
...
{dp}`irm_BwLFrBOljtyA` {ts}`unmapped_with_rationale`
...
{dp}`irm_u8eNzqfz8rTc` {ts}`unmapped_with_rationale`
...
{dp}`irm_7jq6R5X9dhua` {ts}`unmapped_with_rationale`
...
{dp}`irm_KeDMdtOogwD4` {ts}`unmapped_with_rationale`
...
{dp}`irm_DLO3V1Zv7F4z` {ts}`unmapped_with_rationale`
...
{dp}`irm_KYF5r51VaY95` {ts}`unmapped_with_rationale`
...
{dp}`irm_iyAGBy5Law6k` {ts}`unmapped_with_rationale`
...
{dp}`irm_Pv3ani3GyVP9` {ts}`unmapped_with_rationale`
...
{dp}`irm_Wihgut0ZlcE0` {ts}`unmapped_with_rationale`
...
{dp}`irm_RbsN2qwspvj5` {ts}`unmapped_with_rationale`
...
{dp}`irm_25BCZzMbE9Ib` {ts}`unmapped_with_rationale`
...
{dp}`irm_a6CyvCEQXaLd` {ts}`unmapped_with_rationale`
...
{dp}`irm_tKWO3IRxFBHo` {ts}`unmapped_with_rationale`
...
{dp}`irm_RtqNO4A3UlNZ` {ts}`unmapped_with_rationale`
...
{dp}`irm_wqSbEXAFaXZ5` {ts}`unmapped_with_rationale`
Decision procedure (total mapping for every std/core/alloc function and method):

{dp}`irm_4CiUCVkh5nNP` {ts}`unmapped_with_rationale`
...
{dp}`irm_UVFEo8At9U3G` {ts}`unmapped_with_rationale`
...
{dp}`irm_t1yFSS4BWFcF` {ts}`unmapped_with_rationale`
...
{dp}`irm_ZFkQuWxAPXRK` {ts}`unmapped_with_rationale`
...
{dp}`irm_C7neodDb3MNP` {ts}`unmapped_with_rationale`
...
{dp}`irm_WgJRcrxbtTth` {ts}`unmapped_with_rationale`
...
{dp}`irm_xatTXwvDtvV1` {ts}`unmapped_with_rationale`
...
{dp}`irm_C6es3zDk4X61` {ts}`unmapped_with_rationale`
...
{dp}`irm_JputPtdwfANC` {ts}`unmapped_with_rationale`
...
{dp}`irm_TKi2k8zrn9Yd` {ts}`unmapped_with_rationale`
...
{dp}`irm_dybAhalvercg` {ts}`unmapped_with_rationale`
...
{dp}`irm_08YGYnbmkyFB` {ts}`unmapped_with_rationale`

...
def _validate_publish_gates(control_run_root: Path) -> None:
    normalize_summary_path = (
        control_run_root / "artifacts" / "normalize" / "normalize-summary.json"
    )
    ...
    decisions_path = (
        control_run_root / "artifacts" / "extract" / "extract-page-decisions.jsonl"
    )
    for decision in _load_jsonl(decisions_path):
        if decision.get("method") == "ocr_fallback":
            ...
            if quality in {"needs_review", "fail"}:
                raise PublishError(
                    "publish blocked by unresolved OCR quality for "
                    f"{decision.get('part')} page {decision.get('page')}"
                )
...
def run_publish_stage(ctx: "StageContext") -> "StageResult":
    ...
    begin_marker.write_text(
        f"run_id={ctx.run_id}\ntimestamp_utc={utc_now()}\n", encoding="utf-8"
    )

    corpus_root = (
        ctx.paths.repo_root / "traceability" / "iso26262" / "corpus" / "2018-ed2"
    )
    ...
    for part, records in sorted(by_part.items()):
        records.sort(
            key=lambda row: (row["source_locator"]["page_start"], row["anchor_id"])
        )
        ...
        for unit_type in ("paragraph", "list_bullet", "table_cell"):
            ...
            for offset in range(0, len(typed_records), shard_size):
                ...
                shard_name = f"{unit_type}-{shard_idx:04d}.jsonl"
                shard_path = part_dir / shard_name
                _write_jsonl(shard_path, typed_records[offset : offset + shard_size])
        ...
        write_json(
            clause_manifest,
            {
                ...
                "edition": "2018-ed2",
                "clauses": [
                    {"clause": clause, "unit_count": count}
        ...
        part_manifests.append(
            {
                "part": part,
                "manifest": f"corpus/2018-ed2/{part.lower()}/part-manifest.jsonc",
            }
        )
